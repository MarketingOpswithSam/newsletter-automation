...
**Known cloud-sandbox limitation (confirmed 2026-08-24):** from the cloud routine's sandboxed network, direct `curl`/`WebFetch` to ANY `canva.com` or `*.canva.com` host is blocked by the egress proxy with a 403 policy denial, this includes `design.canva.ai` (the `thumbnail.url` host) and `export-download.canva.com` (the signed URL returned by the `export-design` MCP tool). Confirmed via `curl -sS "$HTTPS_PROXY/__agentproxy/status"`, which logs these as `connect_rejected` gateway 403s, and per `/root/.ccr/README.md` a 403/407 from the proxy is a policy denial to report, not something to retry or route around. The Canva MCP tool calls themselves (`generate-design`, `create-design-from-candidate`, `export-design`) work fine since MCP calls bypass the proxy entirely, same as Resend, only the direct-download step fails. This also means candidates can't be visually reviewed by fetching `thumbnail.url` from this sandbox, so that QC step in the paragraph above is only possible from a machine with normal network access (e.g. a local dev session). **When running from the cloud sandbox: skip the image download/commit step entirely and leave `image_url` empty for all sections** (per the Edge Cases entry on image generation failing), rather than losing time on it, this is a standing constraint of this environment, not a one-off flake. Note in the day's log entry that images were skipped and why.

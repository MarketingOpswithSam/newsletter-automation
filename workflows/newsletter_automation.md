...
`generate-design` returns 4 candidates, each with a `thumbnail.url`. Canva is noticeably less reliable than the old Higgsfield path: expect occasional garbled/duplicated words in the banner text or off-brand colors.

**Cloud-sandbox network constraint (confirmed 2026-08-24, re-confirmed 2026-08-26): every `canva.com`/`*.canva.com` host, including `design.canva.ai` (the `thumbnail.url` host) and `export-download.canva.com` (the `export-design` download host), is blocked with a hard `403 connect_rejected` by this account's egress proxy policy for this environment.** This is an organization-level policy denial (verified via `curl -sS "$HTTPS_PROXY/__agentproxy/status"`), not a bug or a flake, and per `/root/.ccr/README.md` it must never be retried or routed around from inside the sandbox. Two consequences follow directly from this:
- The cloud routine **cannot fetch `thumbnail.url` itself**, so it cannot visually QC the 4 candidates the way a local/interactive session could. **Deterministically use candidate index 0** (the first item in the returned list) without attempting to view any of them. Accept the occasional garbled-banner risk noted above as a known tradeoff of running unattended in this environment; if it happens, it's still better than no image at all.
- The cloud routine **cannot download the chosen image to disk either**, so the old approach (curl it into `assets/newsletter_images/`, commit, push, reference the `raw.githubusercontent.com` URL) is a dead end here and must not be attempted.

**The fix: don't download or self-host the image at all. Use candidate 0's `thumbnail.url` directly as the section's `image_url` in the content JSON spec, verbatim, unmodified.** The recipient's own mail client fetches that URL when they open the email, on their own device, completely outside this sandbox's network policy, so the proxy block never comes into play. The tradeoff: `thumbnail.url` is a signed URL that expires roughly a day after generation. Since this is a same-day personal digest (the recipient is expected to read it within a day of send, not archive-browse it weeks later), this is an acceptable tradeoff, not a real risk, expiry just means an image could show as broken if opened unusually late, the text content is unaffected either way.

`assets/newsletter_images/` and the git-commit-images step are now unused for cloud runs. A local/interactive session with normal network access may still choose to view all 4 candidates, pick the best, download, and self-host in the repo for a non-expiring URL if a specific issue calls for it, that path still works fine outside the sandbox, but it is no longer the standard path and the daily cloud run must not attempt it.
